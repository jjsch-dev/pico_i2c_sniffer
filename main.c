...
#if defined(RAM_FIFO) 
    #include "ram_fifo.h"
#elif defined(PICO_QUEUE)
    #include "pico/util/queue.h"
    static queue_t i2c_fifo;
#endif

#undef PRINT_VAL

#undef PRINT_TIME_T
#define PRINT_HEX_INDEX
...
static inline bool fifo_init(size_t count){
#if defined(RAM_FIFO) 
    ram_fifo_init(count);
#elif defined(PICO_QUEUE)
    queue_init(&i2c_fifo, sizeof(uint32_t), count);
#endif
}

static inline bool fifo_is_empty(void){
#if defined(RAM_FIFO)
    return ram_fifo_is_empty();
#elif defined(PICO_QUEUE)
    return queue_is_empty(&i2c_fifo);
#endif  
}

static inline void fifo_set(uint32_t item){
#if defined(RAM_FIFO)
    ram_fifo_set(item);
#elif defined(PICO_QUEUE)
    queue_try_add(&i2c_fifo, &item);
#endif
}

static inline uint32_t fifo_get(void){
uint32_t item;

#if defined(RAM_FIFO)                
    item = ram_fifo_get();
#elif defined(PICO_QUEUE)
    queue_try_remove(&i2c_fifo, &item);
#endif 
...
int main()
{
    ...
    while (true) {
        ...
        if (multicore_fifo_wready()) {
            if (!fifo_is_empty()) {
                if (new_val) {
                    fifo_set(capture_val);
                }
            
                capture_val = fifo_get();
                new_val = true;
            }    
